...
public class WebhookController {
    ...
    private void markAsSigned(String documentId) {
        ...
        if (req == null) {
            ...
            System.out.println("[Webhook]   Make sure boldsign_document_id was saved when the contract was sent.");
            return;
        }
        System.out.println("[Webhook] ✓ Found ContractRequest #" + req.getId()
                + " (current status: " + req.getStatus() + ")");

        // Direct targeted UPDATE — thread-safe fresh connection
        boolean ok = contractRequestDAO.updateStatus(req.getId(), RequestStatus.SIGNED);
        System.out.println("[Webhook] Request #" + req.getId()
